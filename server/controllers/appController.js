...
const getAppointments = async (req, res) => {
  try {
    ...
    const decryptedResults = results.map((request) => ({
      ...request,
...
const getUserAppointments = async (req, res) => {
  ...
  try {
    ...
    console.log("Query results:", decryptedResults);
    res.json(decryptedResults);
  } catch (error) {
...
const markAsComplete = async (req, res) => {
  ...
  try {
    ...
    if (result.affectedRows > 0) {
      res
        .status(200)
        .json({ message: "Appointment marked as complete successfully." });
    } else {
      res.status(404).json({ error: "Appointment not found." });
    }
  } catch (error) {
...
const getMessages = async (req, res) => {
  ...
  if (!userId || !chatPartnerId) {
    return res
      .status(400)
      .json({ error: "Both userId and chatPartnerId are required." });
  }
...
const sendCounselorMessage = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      ...
      sender_id,
      receiver_id,
      message_text, // return plaintext to React
      sent_at: new Date().toISOString(),
    });
  } catch (error) {
...
const getAppointmentsByStudentId = async (req, res) => {
  ...
  try {
    const query =
      "SELECT * FROM requests WHERE student_id = ? ORDER BY requested_at DESC";
